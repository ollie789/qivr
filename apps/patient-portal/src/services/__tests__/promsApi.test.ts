...
describe('promsApi', () => {
  ...
  it('fetchPromInstances unwraps envelope responses', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith('/api/v1/proms/instances', { status: 'pending' });
  ...
  it('fetchPromInstances returns plain values when no envelope present', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith('/api/v1/proms/instances', undefined);
  ...
  it('submitPromResponse posts answers payload', async () => {
    ...
    expect(mockClient.post).toHaveBeenCalledWith('/api/v1/proms/instances/instance-1/answers', {
      answers: [
  ...
  it('savePromDraft forwards draft payload', async () => {
    ...
    expect(mockClient.put).toHaveBeenCalledWith('/api/v1/proms/instances/instance-2/draft', {
      responses: { q1: 'draft' },
      lastQuestionIndex: 2,
    });
  });
  ...
  it('fetchPromStats unwraps responses', async () => {
    mockClient.get.mockResolvedValueOnce({ data: [...apiInstances, ...completedApiInstances] });
