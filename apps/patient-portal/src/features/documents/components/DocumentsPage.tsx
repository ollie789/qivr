...
const DocumentsPage: React.FC = () => {
  ...
  const { data: documents = EMPTY_SUMMARY, isLoading: documentsLoading } =
  ...
  const uploadMutation = useMutation({
    mutationFn: (input: UploadDocumentInput) =>
      uploadDocument(input, (percent) => setUploadProgress(percent ?? 0)),
    onSuccess: () => {
  ...
  const shareMutation = useMutation({
    ...
    onError: () =>
      enqueueSnackbar("Unable to share document", { variant: "error" }),
  });

  const requestReviewMutation = useMutation({
    mutationFn: (documentId: string) =>
      requestDocumentReview(documentId, {
        notes: "Review requested via patient portal",
      }),
    onSuccess: () => {
    ...
    onError: () =>
      enqueueSnackbar("Unable to request review", { variant: "error" }),
  });

  const completeReviewMutation = useMutation({
    mutationFn: (documentId: string) =>
      completeDocumentReview(documentId, {
        notes: "Reviewed via patient portal",
        status: "completed",
      }),
    onSuccess: () => {
    ...
    onError: () =>
      enqueueSnackbar("Unable to mark document as reviewed", {
        variant: "error",
      }),
  });
  ...
  const stats = useMemo(
    () => ({
      ...
      shared: documents.filter((doc) =>
        doc.shares.some((share) => !share.revoked),
      ).length,
    }),
    [documents],
  );
          ...
          <Button
            ...
            onClick={() => {
              if (!shareDialog.userId) {
                enqueueSnackbar("Recipient user ID is required", {
                  variant: "warning",
                });
                return;
              }
