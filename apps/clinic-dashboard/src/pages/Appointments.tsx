...
const appointmentTypes = [
  {
    ...
    duration: 60,
    color: "var(--qivr-palette-primary-main)",
  },
  {
    value: "follow-up",
    label: "Follow-up",
    duration: 30,
    color: "var(--qivr-palette-success-main)",
  },
  {
    value: "assessment",
    label: "Assessment",
    duration: 45,
    color: "var(--qivr-palette-secondary-main)",
  },
  {
    value: "treatment",
    label: "Treatment",
    duration: 30,
    color: "var(--qivr-palette-error-main)",
  },
  {
    value: "review",
    label: "Review",
    duration: 20,
    color: "var(--qivr-palette-warning-main)",
  },
];
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box>
        ...
        <FilterChips
          filters={[
            ...(selectedProviderFilter !== "all"
              ? [
                  {
                    key: "provider",
                    label: `Provider: ${providerOptions.find((p) => p.id === selectedProviderFilter)?.fullName || "Selected"}`,
                  },
                ]
              : []),
            ...(statusFilter !== "all"
          ...
          onRemove={(key) => {
            if (key === "provider") setSelectedProviderFilter("all");
                    ...
                    <FullCalendar
                      ...
                      eventClick={(info) => {
                        setSelectedAppointment(
                          info.event.extendedProps as Appointment,
                        );
                      }}
                      dateClick={(info) => {
                      ...
                      eventDrop={async (info) => {
                        try {
                          ...
                          await apiClient.put(
                            `/api/appointments/${info.event.id}`,
                            {
                      ...
                      eventResize={async (info) => {
                        try {
                          ...
                          enqueueSnackbar("Appointment duration updated", {
                            variant: "success",
                          });
                        } catch (error) {
                          info.revert();
                          enqueueSnackbar("Failed to update appointment", {
                            variant: "error",
                          });
                        }
                      }}
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid size={12}>
                <Autocomplete
                  ...
                  getOptionLabel={(option) =>
                    option.fullName || option.email || "Provider"
                  }
                  isOptionEqualToValue={(option, value) =>
                    option.id === value.id
                  }
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      if (appointmentType) {
                        ...
                        endTime.setMinutes(
                          endTime.getMinutes() + appointmentType.duration,
                        );
                        setNewAppointment({
                          ...newAppointment,
              ...
              <Stack spacing={2} sx={{ mt: 2 }}>
                ...
                <Box>
                  ...
                  <Typography>
                    ...
                    -
                    {format(
                      parseISO(selectedAppointment.scheduledEnd),
                      "h:mm a",
                    )}
                  </Typography>
                  <Typography variant="body2">
                    {
                      appointmentTypes.find(
                        (t) => t.value === selectedAppointment.appointmentType,
                      )?.label
                    }
                  </Typography>
                </Box>
