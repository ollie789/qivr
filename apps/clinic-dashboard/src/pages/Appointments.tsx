...
import { TreatmentPlanBuilder, PaymentRecordDialog } from '../components/dialogs';
import { SessionView } from '../components/session';
import { AppointmentPopover } from '../components/appointments';
...
export default function Appointments() {
  ...
  const [patientTreatmentPlan, setPatientTreatmentPlan] = useState<TreatmentPlanSummary | null>(
    null
  );
  ...
  const [sessionViewAppointment, setSessionViewAppointment] = useState<Appointment | null>(null);
  ...
  const { data: providers = [] } = useQuery({
    queryKey: ['providers'],
    queryFn: () => providerApi.getClinicProviders(undefined, { activeOnly: true }),
  });
  ...
  const appointments = useMemo(() => {
    ...
    return allAppointments.filter((apt) => apt.providerId === selectedProviderId);
  ...
  const handleEventClick = (info: EventClickArg) => {
    ...
    // In week view: open popover for details
    // In month view: select the day for agenda
    if (viewMode === 'week') {
      // If appointment is in-progress, open SessionView directly
      if (apt.status === 'in-progress') {
        setSessionViewAppointment(apt);
      } else {
        // Open popover with appointment details
        setPopoverAnchor({ el: info.el as HTMLElement, apt });
      }
    } else {
      // Month view: clicking appointment selects that day for agenda
      const aptDate = parseISO(apt.scheduledStart);
      setAgendaDate(aptDate);
    }
  };
  ...
  const loadAppointmentSession = async (apt: Appointment) => {
    ...
    const appointmentTypeKey = apt.appointmentType?.toLowerCase().replace(/[^a-z]/g, '') || '';
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      if (
        ...
        patientTreatmentPlan.nextSessionNumber < patientTreatmentPlan.totalSessions
      ) {
  ...
  return (
    ...
    >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          >
            <ToggleButton value="week">
              <WeekViewIcon sx={{ mr: 0.5, fontSize: 16 }} />
              Week
            </ToggleButton>
            <ToggleButton value="month">
              <MonthViewIcon sx={{ mr: 0.5, fontSize: 16 }} />
              Month
            </ToggleButton>
          </ToggleButtonGroup>
          ...
          <Typography variant="h6" fontWeight={600} sx={{ ml: 1, color: 'text.primary' }}>
      ...
      >
        ...
        >
          <Box
            sx={{
              flex: 1,
              // FullCalendar grid line overrides - softer styling
              '& .fc': {
                '--fc-border-color':
              ...
              // Week view event styling with status bar
              '& .fc-timegrid-event': {
                borderRadius: '6px',
                fontSize: '0.75rem',
              ...
              // === COLUMN HEADERS ===
              '& .fc-col-header': {
                borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)} !important`,
              },
              '& .fc-col-header-cell': {
                py: 1.5,
                verticalAlign: 'top',
                borderRight: `1px solid ${alpha(theme.palette.divider, 0.3)} !important`,
              ...
              '& .fc-timegrid-slot-minor': {
                ...
                borderTopColor: `${alpha(theme.palette.divider, 0.25)} !important`,
              },
              // Time labels
                  ...
                  return (
                    <Paper
                      ...
                      sx={{
                        borderRadius: '8px',
                        border: '1px solid',
                        borderColor: isActive ? 'warning.main' : alpha(theme.palette.divider, 0.8),
                        bgcolor: isActive
                        ...
                        display: 'flex',
                        '&:hover': {
                          borderColor: isActive ? 'warning.main' : theme.palette.primary.main,
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <Chip
                            ...
                            sx={{
                              ...
                              fontSize: '0.65rem',
                              fontWeight: 600,
                              bgcolor: alpha(getStatusColor(apt.status), 0.12),
                        ...
                        {/* Patient Name */}
                        <Typography variant="subtitle2" fontWeight={600} noWrap sx={{ mb: 0.25 }}>
                          {apt.patientName}
                        </Typography>
                        ...
                        >
                          ...
                          <AuraIconButton
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(`/medical-records?patientId=${apt.patientId}&tab=timeline`);
                            }}
                            sx={{
      ...
      {/* Appointment Popover (Week View) */}
      <AppointmentPopover
        appointment={popoverAnchor?.apt ?? null}
        anchorEl={popoverAnchor?.el ?? null}
        open={Boolean(popoverAnchor)}
        onClose={() => setPopoverAnchor(null)}
        onStartSession={(apt) => handleStartSession(apt)}
        onViewPatient={(patientId) =>
          navigate(`/medical-records?patientId=${patientId}&tab=timeline`)
        }
        onReschedule={(apt) => {
          setSelectedDate(parseISO(apt.scheduledStart));
          setScheduleDialogOpen(true);
        }}
        onMoreOptions={(e, apt) => {
          setMenuAnchor({ el: e.currentTarget, apt });
        }}
      />
      ...
      >
        ...
        >
          <ListItemIcon>
            <PaymentIcon fontSize="small" color={menuAnchor?.apt?.isPaid ? 'success' : 'primary'} />
          </ListItemIcon>
          <ListItemText>{menuAnchor?.apt?.isPaid ? 'View Payment' : 'Record Payment'}</ListItemText>
        </MenuItem>
        <Divider />
        ...
        <Stack spacing={auraTokens.spacing.lg}>
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary">
              {selectedAppointment &&
                format(parseISO(selectedAppointment.scheduledStart), "EEEE, MMMM d 'at' h:mm a")}
            </Typography>
            ...
            {loadingPlan ? (
              ...
              <Stack spacing={1.5}>
                ...
                <FormControlLabel
                  ...
                  label={
                    <Typography variant="body2">Send home exercise reminder to patient</Typography>
                  }
                />
      ...
      <FormDialog
        ...
        actions={
          ...
          >
            <AuraButton onClick={() => setScheduleNextSessionOpen(false)}>Skip for Now</AuraButton>
            <AuraButton
              variant="contained"
