...
const Messages: React.FC = () => {
  ...
  const selectedConversation: ConversationSummary | null = useMemo(() => {
    ...
    return (
      source.find(
        (conversation) => conversation.participantId === selectedConversationId,
      ) ?? null
    );
  }, [conversationsData, selectedConversationId]);
  ...
  const visibleMessages: MessageDetail[] = useMemo(() => {
    ...
    return conversationMessages.filter(
      (message) => message.messageType.toLowerCase() === needle,
    );
  ...
  const latestMessageType = useMemo(() => {
    ...
    return lastMessage?.messageType?.toLowerCase() === "email"
      ? "email"
      : "sms";
          ...
          {conversationsLoading ? (
            ...
            <List sx={{ p: 0 }}>
              {conversations.map((conversation) => {
                const isSelected =
                  conversation.participantId === selectedConversationId;
            ...
            ? [
                {
                  ...
                  type: selectedConversation.participantRole
                    .toLowerCase()
                    .includes("patient")
                    ? "patient"
                    : "staff",
