...
function BodyRegionMesh({
  ...
  const config = BODY_REGION_CONFIG[region];

  if (!config) return null;

  const color = isAffected ? getIntensityColorThree(intensity) : 0xe2e8f0;
  const opacity = isAffected ? 0.5 + (intensity / 10) * 0.4 : 0.25;
  const emissiveIntensity = isAffected ? 0.2 + (intensity / 10) * 0.4 : 0;

  // Mirror for arms and legs
  const isMirrored = ['Arm', 'Hand', 'Leg', 'Knee', 'Ankle', 'Foot'].includes(region);

  const renderMesh = (position: [number, number, number], key: string) => {
    if (config.type === 'sphere') {
      return (
        <mesh key={key} ref={meshRef} position={position}>
          <sphereGeometry args={[config.scale[0], 16, 16]} />
    ...
    return (
      <mesh key={key} ref={meshRef} position={position}>
        <capsuleGeometry args={[config.scale[0], config.scale[1] * 2, 8, 16]} />
        <meshStandardMaterial
          color={color}
          transparent
          opacity={opacity}
  ...
  if (isMirrored) {
    return (
      <>
        {renderMesh(config.position, `${region}-right`)}
        {renderMesh([-config.position[0], config.position[1], config.position[2]], `${region}-left`)}
      </>
    );
  }

  return renderMesh(config.position, region);
}
