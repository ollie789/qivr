...
const TreatmentTabContent: React.FC<TreatmentTabContentProps> = ({ patientId, patient }) => {
  ...
  if (plans.length === 0) {
    return (
      <Box sx={{ p: auraTokens.spacing.xl, textAlign: 'center' }}>
        ...
        <AuraButton variant="contained" onClick={() => setAssignDialogOpen(true)}>
  ...
  return (
    <Box sx={{ p: auraTokens.spacing.lg }}>
      ...
      >
        ...
        <AuraButton size="small" variant="outlined" onClick={() => setAssignDialogOpen(true)}>
          Assign New Plan
        </AuraButton>
      </Box>
...
const TAB_NAMES = ['demographics', 'history', 'treatment', 'documents', 'referrals'] as const;
...
const MedicalRecordsPage: React.FC = () => {
  ...
  const { data: referrals = [], isLoading: isReferralsLoading } = usePatientReferrals(
    shouldFetchReferrals ? selectedPatientId : null
  );
  ...
  // New clinical summary data
  const { data: intakeData } = usePatientIntake(shouldFetchHistory ? selectedPatientId : null);
  const { data: promResponses = [] } = usePatientPromResponses(
    shouldFetchHistory ? selectedPatientId : null
  );
  ...
  const addHistoryMutation = useMutation({
    mutationFn: async () => {
      ...
      if (
        ['injury', 'symptom', 'treatment', 'activity', 'occupation', 'goal'].includes(
          newHistory.category || ''
        )
      ) {
    ...
    onSuccess: () => {
      ...
      enqueueSnackbar(`${category.charAt(0).toUpperCase() + category.slice(1)} added`, {
        variant: 'success',
      });
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      >
          ...
          >
            ...
            {patient && (
              ...
              >
                <Tabs value={activeTab} onChange={handleTabChange}>
                  ...
                  <Tab icon={<MedicalIcon />} iconPosition="start" label="Clinical Summary" />
                  <Tab icon={<TreatmentIcon />} iconPosition="start" label="Treatment" />
                  <Tab icon={<DocumentIcon />} iconPosition="start" label="Documents" />
                  <Tab icon={<ReferralIcon />} iconPosition="start" label="Referrals" />
                </Tabs>
              </Box>
            )}
            ...
            <Box sx={{ flex: 1, overflow: 'auto', bgcolor: alpha('#f8fafc', 0.5) }}>
              ...
              <TabPanel value={activeTab} index={2}>
                <TreatmentTabContent patientId={selectedPatientId} patient={patient} />
              </TabPanel>
              ...
              <TabPanel value={activeTab} index={4}>
                {isTabLoading ? (
                  ...
                  <ReferralsTab referrals={referrals} patientId={selectedPatientId} />
                )}
              </TabPanel>
            </Box>
          </Box>
          ...
          <Grid container spacing={3} sx={{ mt: 1 }}>
            ...
            <Grid size={{ xs: 12, md: 7 }}>
              <PainMap3D
                ...
                onChange={(regions) => setNewVital({ ...newVital, painRegions: regions })}
              />
            </Grid>
          </Grid>
        </FormDialog>
        ...
        >
          ...
          <DialogContent>
            Are you sure you want to delete &ldquo;{itemToDelete?.title}&rdquo;? This action cannot
            be undone.
          </DialogContent>
          <DialogActions>
