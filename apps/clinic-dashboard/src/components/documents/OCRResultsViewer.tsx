...
export default function OCRResultsViewer({ document }: OCRResultsViewerProps) {
  const getStatusIcon = () => {
    ...
    if (document.status === 'ready' && document.confidenceScore && document.confidenceScore > 80) {
      return <CheckCircle color="success" />;
    }
    if (document.status === 'ready' && document.confidenceScore && document.confidenceScore > 60) {
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
          ...
          {document.confidenceScore && (
            <Chip
              ...
              size="small"
              color={document.confidenceScore > 80 ? 'success' : document.confidenceScore > 60 ? 'warning' : 'error'}
            />
          )}
        </Box>
        ...
        {document.status === 'ready' && (
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Typography variant="body1" fontWeight={500}>
                {document.extractedDob ? new Date(document.extractedDob).toLocaleDateString() : 'Not detected'}
              </Typography>
            </Grid>
