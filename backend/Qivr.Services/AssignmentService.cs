...
public class AssignmentService : IAssignmentService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Check if provider exists and is active - FIXED SQL INJECTION
        ...
        CancellationToken ct = default)
    {
        ...
        // Get current assignment - FIXED SQL INJECTION
        ...
        CancellationToken ct = default)
    {
        ...
        // Build safe query with parameters - FIXED SQL INJECTION
        var query = @"SELECT 
        ...
        if (status.HasValue)
        {
            ...
            var assignments = await _db.Database
                .SqlQueryRaw<AssignmentDto>(query + " ORDER BY a.assigned_at DESC",
                    tenantId, providerId, status.Value.ToString().ToLowerInvariant())
                .ToListAsync(ct);
            return assignments;
        }
        else
        ...
        CancellationToken ct = default)
    {
        ...
        var workloads = await _db.Database
            .SqlQueryRaw<ProviderWorkloadDto>(
                @"SELECT 
                    u.id as ProviderId,
                    u.first_name || ' ' || u.last_name as ProviderName,
