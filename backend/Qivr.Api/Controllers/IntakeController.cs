                ...
                await using (var findCmd = conn.CreateCommand())
                {
                    ...
                    else
                    {
                        ...
                        insertCmd.CommandText = @"INSERT INTO qivr.users (
                                id, tenant_id, cognito_sub, email, email_verified, phone_verified, first_name, last_name, phone, user_type, roles, preferences, created_at, updated_at
                            ) VALUES (
                                @id, @tenant, @cognito, @email, false, false, @first, @last, @phone, 0, ARRAY[]::text[], '{}'::jsonb, NOW(), NOW()
                            ) RETURNING id";
