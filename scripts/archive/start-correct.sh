...
# QIVR Services Startup Script - CORRECT PORTS
# ================================================
# Backend API:      5050 (Avoids macOS AirPlay on 5000)
# Patient Portal:   3005 (User preference)
# Clinic Dashboard: 3010 (User preference)
# Widget:           3003
# ================================================
...
kill_port 3003  # Widget
# Also kill any old/default ports that might be in use
...
dotnet run --project Qivr.Api --urls "http://localhost:5050" > "$PROJECT_ROOT/logs/backend.log" 2>&1 &
BACKEND_PID=$!
...
if [ -d "$PROJECT_ROOT/apps/widget" ]; then
    ...
    npx vite --port 3003 > "$PROJECT_ROOT/logs/widget.log" 2>&1 &
    WIDGET_PID=$!
...
echo -e "${YELLOW}To stop all services, run: pkill -f 'dotnet|vite'${NC}"
...
[ -n "$WIDGET_PID" ] && echo "$WIDGET_PID" > "$PROJECT_ROOT/.pids/widget.pid"
